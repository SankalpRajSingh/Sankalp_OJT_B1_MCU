...
int main(void)
{
  ...
  /* USER CODE BEGIN WHILE */
	while(1)
	{
		HAL_UART_Receive(&huart2, &recvd_data, 1, HAL_MAX_DELAY);    //Read byte by byte
...
//			Data_Buffer[count++] = Convert_To_Upper(recvd_data);
  ...
  /* USER CODE END 3 */
	...
	else if(strcmp(Data_Buffer,"3E")==0)
	{
		LCD_command(1);
		LCD_string("WELCOME ON BOARD");
		LCD_command(0xC4);
	...
	else if(strcmp(Data_Buffer,"55")==0)
	{
		LCD_command(1);
		LCD_string("WELCOME ON BOARD");
		LCD_command(0xC4);
